...
export default function OnboardingPage() {
  ...
  const [tipoCampana, setTipoCampana] = useState('bienvenida');
  ...
  const fetchMisCampanas = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/onboarding/mis-campanas`, {
  ...
  const handleVerCampana = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(`${API_URL}/onboarding/${id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleGenerar = async () => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(`${API_URL}/onboarding/generar`, {
        method: 'POST',
        headers: {
  ...
  const handleEnviarAprobacion = async () => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(`${API_URL}/onboarding/${resultado.campana_id}/enviar-aprobacion`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleEnviarAprobacionHistorial = async (campanaId: string) => {
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(`${API_URL}/onboarding/${campanaId}/enviar-aprobacion`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleEliminarBorrador = async (campanaId: string) => {
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(`${API_URL}/onboarding/${campanaId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleRefinar = async () => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      // Concatenar el ajuste a la instrucción original
      ...
      const res = await fetch(`${API_URL}/onboarding/generar`, {
        ...
        body: JSON.stringify({
          ...
          tipo_campana: campanaView.tipo_campana || 'bienvenida',
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.detail || 'Error al refinar la campaña');
      }

      const data = await res.json();
      
      // Actualizamos la vista local con la nueva data
      setCampanaView({
        ...campanaView,
  ...
  const handleRefinarPreview = async () => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      // Usamos la instrucción actual del text area como base, ya que es lo que el usuario ve
      ...
      // Actualizamos la caja grande para que contenga el historial del prompt
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6 lg:col-span-2">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
            <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-4 tracking-tight border-b pb-2">PASO 1 — Describe tu campaña</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full p-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm"
                >
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Tipo de campaña:</label>
                <div className="flex space-x-4">
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="radio" value="bienvenida" checked={tipoCampana === 'bienvenida'} onChange={(e) => setTipoCampana(e.target.value)} className="text-emerald-600 focus:ring-emerald-500" />
                    <span className="text-sm text-gray-800 dark:text-gray-200">Bienvenida</span>
                  </label>
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="radio" value="activacion" checked={tipoCampana === 'activacion'} onChange={(e) => setTipoCampana(e.target.value)} className="text-emerald-600 focus:ring-emerald-500" />
                    <span className="text-sm text-gray-800 dark:text-gray-200">Activación</span>
                  </label>
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="radio" value="seguimiento" checked={tipoCampana === 'seguimiento'} onChange={(e) => setTipoCampana(e.target.value)} className="text-emerald-600 focus:ring-emerald-500" />
                    <span className="text-sm text-gray-800 dark:text-gray-200">Seguim.</span>
                  </label>
                </div>
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Instrucción en lenguaje natural:</label>
                <textarea
                  className="w-full h-32 p-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm"
                  placeholder="Ej: Generar campaña de bienvenida para prospectos que abrieron cuenta hace más de 3 días y aún no han realizado su primera transacción..."
                  value={instruccion}
              ...
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center px-4 py-3 border border-transparent text-sm font-bold rounded-lg shadow-sm text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 disabled:opacity-50 transition-colors"
              >
          ...
          {/* RESULTADO PREVIEW */}
          {resultado && !loading && (
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-emerald-100 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-200 flex flex-col">
                  ...
                  <div className="mb-3">
                    ...
                    <span className="text-sm text-indigo-900 ml-1">prospectos</span>
                  </div>
                  ...
                  <div className="space-y-1 text-xs text-indigo-800 pt-3 border-t border-indigo-200">
                    ...
                    <p><span className="font-semibold">Tipo:</span> {resultado.tipo_campana || tipoCampana}</p>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      className="block w-full border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm"
                      disabled={refinandoPreview}
                    />
                    <button
                      onClick={handleRefinarPreview}
                      disabled={refinandoPreview || !refinarTextoPreview.trim()}
                      className="inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50"
                    >
              ...
              {estadoAprobacion === 'success' ? (
                <div className="mt-4 flex items-center justify-center text-emerald-700 bg-emerald-50 px-4 py-3 rounded-xl font-bold text-sm border border-emerald-200">
                ...
                <>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="flex-[2] flex items-center justify-center px-4 py-3 border border-transparent shadow-sm text-sm font-bold rounded-xl text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none transition-colors disabled:opacity-50"
                    >
            ...
            <div className="flex-shrink-0 mb-4">
              ...
              <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                <button
                  ...
                  className={`flex-1 py-1.5 text-sm font-semibold rounded-md transition-all ${activeTab === 'borradores' ? 'bg-white dark:bg-gray-800 text-emerald-700 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 dark:text-gray-300'}`}
                >
                  Borradores
                </button>
                <button
                  onClick={() => setActiveTab('historial')}
                  className={`flex-1 py-1.5 text-sm font-semibold rounded-md transition-all ${activeTab === 'historial' ? 'bg-white dark:bg-gray-800 text-emerald-700 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 dark:text-gray-300'}`}
                >
                  Enviadas
                </button>
              </div>
                    ...
                    <div key={c.id} className="p-4 border border-gray-100 dark:border-gray-700 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900 transition-colors flex justify-between items-center">
                      <div>
                        <div className="text-sm font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                          {/* We try to extract "tipo" from the instruction if possible, or just print instruction snippet */}
                      ...
                      <button 
                        ...
                        className="text-sm text-emerald-600 font-semibold hover:text-emerald-800 disabled:opacity-50"
                      >
                        Ver →
                      </button>
                    </div>
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    ...
                    <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg mb-4">
                      <button
                        ...
                        className={`flex-1 py-1.5 text-sm font-semibold rounded-md transition-all ${modalTab === 'contenido' ? 'bg-white dark:bg-gray-800 text-emerald-700 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 dark:text-gray-300'}`}
                      >
                        Contenido
                      </button>
                      <button
                        onClick={() => setModalTab('auditoria')}
                        className={`flex-1 py-1.5 text-sm font-semibold rounded-md transition-all ${modalTab === 'auditoria' ? 'bg-white dark:bg-gray-800 text-emerald-700 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 dark:text-gray-300'}`}
                      >
                        Auditoría IA
                      </button>
                    </div>
                      ...
                      <div className="space-y-4">
                        <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
                          ...
                          {campanaView.logs && campanaView.logs.length > 0 ? (
                            (() => {
                              // El log más reciente (última generación)
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                  ...
                                  <div className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col">
                                    ...
                                    {(metricas.tiempo_respuesta_ms / 1000) <= 30 ? (
                                      <span className="text-[10px] font-bold text-emerald-600 mt-1">✓ Óptimo (≤30s)</span>
                                    ) : (
                                  ...
                                  <div className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col">
                                    ...
                                    {metricas.tasa_anonimizacion >= 99.5 ? (
                                      <span className="text-[10px] font-bold text-emerald-600 mt-1">✓ Seguro (≥99.5%)</span>
                                    ) : (
                                  ...
                                  <div className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col">
                                    ...
                                    {metricas.precision_segmento >= 85 ? (
                                      <span className="text-[10px] font-bold text-emerald-600 mt-1">✓ Alta (≥85%)</span>
                                    ) : (
                ...
                <div className="flex flex-col sm:flex-row-reverse gap-2 sm:gap-0">
                  ...
                  {campanaView.estado === 'borrador' && (
                    <button
                      ...
                      className="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 bg-emerald-600 text-base font-medium text-white hover:bg-emerald-700 focus:outline-none sm:w-auto sm:text-sm"
                    >
            ...
            <div className="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-gray-100 dark:border-gray-700">
              ...
              <div className="bg-gray-50 dark:bg-gray-900 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2 sm:gap-0">
                ...
                <button
                  ...
                  className="mt-3 w-full inline-flex justify-center rounded-xl border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-800 text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm transition-colors"
                >
                  Cancelar
                </button>
              </div>
